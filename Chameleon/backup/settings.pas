...
type
  TProgramMode =
    (pmNone, pmBattery, pmTime, pmWeatherConditions, pmWindSpeed,
    pmTemperature, pmHumidity, pmHeatIndex);
...
const
  Folders: array [0..6] of string =
    ('Battery', 'Time', 'WeatherConditions', 'WindSpeed', 'Temperature',
    'Humidity', 'HeatIndex');
...
const
  WindSpeedModes: array [0..11] of string =
    ('0 MPH', '1 to 9 MPH', '10 to 19 MPH', '20 to 29 MPH', '30 to 39 MPH',
    '40 to 49 MPH', '50 to 59 MPH', '60 to 69 MPH',
    '70 to 79 MPH', '80 to 89 MPH', '90 to 99 MPH', '100+ MPH');

const
  TemperatureModes: array [0..20] of string =
    ('<= -50°F', '-49 to -40°F', '-39 to -30°F', '-29 to -20°F',
    '-19 to -10°F', '-9 to -1°F', '0°F', '1 to 9°F', '10 to 19°F',
    '20 to 29°F', '30 to 39°F',
    '40 to 49°F', '50 to 59°F', '60 to 69°F', '70 to 79°F',
    '80 to 89°F', '90 to 99°F', '100 to 109°F', '110 to 119°F',
    '120 to 129°F', '>= 130°F');
...
begin
  ...
  begin
    ...
    try
      ProgramSettings.Mode :=
        TProgramMode(INI.ReadInteger('Program', 'ProgramMode', 0));
      ...
      ProgramSettings.WeatherStationName :=
        INI.ReadString('Weather', 'WeatherStationName', '');
    finally
      INI.Free;
    end;
  end
...
begin
  ...
  try
    ...
    INI.WriteString('Weather', 'WeatherStationName',
      ProgramSettings.WeatherStationName);
...
function GetTemperature(WeatherStationName: string): string;
begin
  Result := ConvertTemperature(GetWeatherByStationName(
    WeatherStationName).Temperature);
end;

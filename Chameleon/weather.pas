...
const
  AllStationsXMLFile: string = 'https://w1.weather.gov/xml/current_obs/index.xml';
...
const
  StateAbreviations: array [0..63] of string =
...
type
  TWeatherStationArray = array of TWeatherStation;
...
function GetStationsForState(AllStations: TWeatherStationArray;
  StateAbbreviation: string): TWeatherStationArray;
function GetStationByName(AllStations: TWeatherStationArray;
  StationName: string): TWeatherStation;
...
begin
  ...
  (*
    ...
    XML data from.
  *)
function CreateWeatherStation(Name: string; State: string;
  XMLURL: string): TWeatherStation;
var
...
begin
  if URLDownloadToFile(nil, PChar(Url), PChar(FilePath), 0, nil) = 0 then
  begin
        ...
        begin
          ...
          with Child.ChildNodes do
            try
              ...
              begin
                if Item[i].NodeName = 'station_name' then
                begin
                  CurrentStation.Name := FormatName(Item[i].FirstChild.NodeValue);
                end
                else if Item[i].NodeName = 'station_id' then
                begin
                  CurrentStation.Id := Item[i].FirstChild.NodeValue;
                end
                else if Item[i].NodeName = 'state' then
                begin
                  CurrentStation.State := Item[i].FirstChild.NodeValue;
                end
                else if Item[i].NodeName = 'xml_url' then
                begin
                  CurrentStation.XMLUrl := Item[i].FirstChild.NodeValue;
                  CurrentStation.XMLUrl :=
                    CurrentStation.XMLUrl.Replace('https:', 'http:', [rfReplaceAll]);
                  CurrentStation.XMLUrl :=
                    CurrentStation.XMLUrl.Replace('//weather.gov',
                    '//w1.weather.gov', [rfReplaceAll]);
                end;
              end;

              // Station is valid
              if CurrentStation.Name <> '' then
              begin
...
var
  ...
  Stations: TWeatherStationArray;
begin
  for CurrentStation in AllStations do
  begin
    if CurrentStation.State = StateAbbreviation then
    begin
      Inc(StationCount);
      SetLength(Stations, StationCount);
  ...
  begin
    try
      ...
      begin
        ...
        if NodeName = 'weather' then
        begin
          Weather.Conditions :=
            NormalizeWeatherCondition(Child.FirstChild.NodeValue);
        end
        else if NodeName = 'temp_f' then
        begin
          Weather.Temperature := Round(Child.FirstChild.NodeValue.ToDouble());
        end
        else if NodeName = 'relative_humidity' then
        begin
          Weather.Humidity := Child.FirstChild.NodeValue.ToInteger();
        end
        else if NodeName = 'wind_mph' then
        begin
          Weather.WindSpeed := Child.FirstChild.NodeValue.ToDouble();
        end
        else if NodeName = 'wind_dir' then
        begin
...
begin
  ...
  begin
    ...
    try
      ...
      for ConditionNamePos := 0 to ConditionNames.Count - 1 do
        if (LowerCase(WeatherCondition) = LowerCase(
          ConditionNames[ConditionNamePos])) then
        begin
  ...
  begin
    if WeatherCondition.ToLower.Contains('thunderstorm') then
    begin
...
begin
  ...
  if (t >= 80) and (t <= 112) then
  begin
    // This is more accurate, but only works between the values above
    Result := Round(c1 + c2 * t + c3 * r + c4 * t * r + c5 * sqr(t) +
      c6 * sqr(r) + c7 * sqr(t) * r + c8 * t * sqr(r) + c9 * sqr(t) * sqr(r));
  end
  else
  begin
